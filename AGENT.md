...
本文件面向所有 AI coding agent，用来快速接手 `weibo_super_stats`。它比普通 README 更偏工程协作：告诉你项目怎么跑、目录怎么分、接口在哪里、数据怎么流动、改哪里、怎么测、哪些边界不能碰。
...
4. `CLAUDE.md`：Claude Code 专用指南，内容和本文件大体对齐。
5. 当前任务涉及的源码文件。

## 项目定位

`weibo_super_stats` 是一个 Windows 本地运行的微博超话周报统计工具。

用户在 WebUI 中填写超话链接或 ID、微博 Cookie、时间窗口和导出目录；后端抓取指定时间范围内的超话帖子，补全文本，分析评论，评分排序；用户在候选列表里人工筛选后，工具导出 Markdown、DOCX、XLSX、CSV、summary、微博正文草稿和适合微博发图的长图。

项目只建议运行在 `127.0.0.1`。不要默认改造成公网服务，也不要默认引入公网部署假设。
...
```text
WebUI -> HTTP API -> CrawlJob -> crawler -> cache -> export -> manifest/history
```
...
- Cookie 辅助：`browser-cookie3`、浏览器调试端口/CDP。
- 导出：`openpyxl`、`python-docx`、`pillow`。
- 长图截图：`playwright` 调用本机 Edge/Chromium。
- 测试：`unittest`。

不要在未被用户明确要求时引入 Flask、FastAPI、React、Vue、Svelte、Node 构建链或新框架。
...
前端 `web/js/task.js` 通过 `/api/status` 轮询，不要随意破坏 job snapshot 结构。
...
- 不要默认提交本地配置、历史、输出、浏览器 profile。

## Agent 协作规则

- 开始改代码前先确认当前 git 状态，避免覆盖用户改动。
- 遇到未追踪或已修改文件，默认视为用户改动，不要删除或回滚。
- 文件编辑使用补丁式改动，保持 diff 小而清楚。
- 不使用 `git reset --hard`、`git checkout --` 等破坏性命令，除非用户明确要求。
- 涉及生成文件、清理 output、删除目录时要非常保守。
- 对用户可见功能做改动时，给出测试和文档同步建议。
