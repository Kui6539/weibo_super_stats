...
- WebUI 支持基础模式 / 高级模式，常用参数默认更简洁。
- 支持自定义统计时间段，默认窗口为最近一次已过的 `04:00` 到上周同刻 `04:00`。
- 点击开始前会执行预检查，明确提示超话、Cookie、时间、导出目录和任务状态是否可用。
- 自动翻页抓取，连续 5 页无时间范围内帖子时停止。
- 支持 Edge / Chrome 调试浏览器自动读取 Cookie，也支持剪贴板读取、粘贴内容与 cURL 片段自动识别。
- Cookie 区域支持折叠、测试 Cookie、清空 Cookie 和本地保存浏览器选择。
...
- 任务状态使用结构化阶段进度显示，详细日志以可拖动的悬浮气泡/窗口呈现。
- 每次任务保存本地 JSON 缓存，导出失败或样式调整后可离线重新生成报告。
...
也可以直接双击 `点我启动.bat`。脚本会优先使用项目内 `.venv`，缺少依赖时按 `requirements.txt` 安装后再启动本地页面。
...
1. 在 Cookie 推荐方式卡片右上角选择 `Edge` 或 `Chrome`，点击 `打开调试浏览器`，在新窗口登录微博，再点击 `自动读取 Cookie`。这是推荐方式，不依赖浏览器本地 Cookie 数据库。
2. 直接点击 `自动读取 Cookie`，程序会优先尝试所选浏览器的 CDP 调试端口，再降级读取本机浏览器 Cookie。
3. 使用 `读取剪贴板` / `识别粘贴内容`，自动识别剪贴板里的 Cookie、请求头或 `Copy as cURL` 片段。
4. 手动打开浏览器开发者工具，从 Network 请求里复制完整 Cookie。

调试浏览器说明：

- Edge 默认使用端口 `9222` 和目录 `.edge_cdp_profile`。
- Chrome 默认使用端口 `9223` 和目录 `.chrome_cdp_profile`。
- 调试窗口和日常浏览器配置分开，第一次打开可能需要重新登录微博。
- 自动读取成功后，工具会尝试自动关闭对应调试浏览器。

更详细的步骤见 [Cookie获取简短教程.md](./Cookie获取简短教程.md)。

注意：Cookie 等同于登录态，请不要提交到 GitHub，也不要发给他人。

工具会把超话链接或 ID、微博 Cookie、导出目录、主题、浏览器选择和日志窗口位置等本地设置保存到项目根目录的 `weibo_stats_config.json`，下次启动会自动读取。该文件已加入 `.gitignore`，不要手动提交。
...
3. 填入超话链接或 ID。
4. 在 Cookie 推荐方式卡片选择 `Edge` 或 `Chrome`，点击 `打开调试浏览器`，在新窗口登录微博后点击 `自动获取 Cookie`；失败时使用 `读取剪贴板` / `识别粘贴内容` 或手动复制。
5. 设置起始与结束日期时间。
6. 点击 `测试 Cookie`，确认不是访客态或失效 Cookie。
7. 根据需要打开高级模式，调整最大翻页页数、话题评论系数、请求间隔和导出目录。
8. 点击 `开始抓取并导出`。
9. 查看预检查结果；有错误时按提示修正，只有警告时可选择继续。
10. 在页面出现的“人工筛选”区域中复核候选帖子。
11. 等待导出完成，在 `output/时间戳/` 文件夹查看结果，也可以点击“打开文件所在位置”。
...
modules/
  ...
  cookie_edge_debug.py # 调试 Edge/Chrome 启动、读取和关闭封装
...
第五期拆分后，`cookie_helper.py` 仍作为兼容入口，对外保留原有函数名；具体的 Cookie 文本解析、调试 Edge/Chrome、本地浏览器存储读取和轻量检测已迁移到 `modules/`。WebUI 保持无框架实现，`web/js/main.js` 主要负责控制器装配和事件绑定；配置读写、表单、高级模式、Cookie 区域、预检查、结构化进度、悬浮任务日志、人工筛选、Markdown 预览、缓存重新生成、帮助弹窗、主题切换、粒子背景和任务轮询分别迁移到 `web/js/` 下的独立模块。
...
- `POST /api/cookie/edge-debug`：按浏览器参数启动 Edge/Chrome 调试窗口，保留旧路径名以兼容前端。
- `POST /api/cookie/auto`：按浏览器参数自动读取微博 Cookie。
- `POST /api/cookie/extract`：从 Cookie 字符串、请求头或 cURL 片段中提取 Cookie。
...
`weibo_stats_config.json` 当前使用 `version=2` 的扁平结构，保存超话、Cookie、导出目录、主题、高级模式、Cookie 浏览器选择和任务日志位置等本地设置。旧配置会在读取时自动迁移；`scope=cookie` 清空时只清 Cookie，`scope=all` 会备份旧配置为 `weibo_stats_config.backup.json` 后恢复默认值。
