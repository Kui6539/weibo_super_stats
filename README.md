...
- 结构化任务状态、阶段进度、任务取消和终端滚动日志；开始任务后会先展示预检查结果，再展示任务状态。
- 抓取帖子、补全文本、评论分析、评分排序和人工筛选；候选卡片会显示入选帖缩略图。
...
- 取消或失败的未完成任务会自动清理对应 `output/<运行ID>/` 和 `cache/<运行ID>/`。
- 输出清理工具支持先预览再确认删除，默认不会自动清理任何已完成文件。
...
如果调试浏览器登录态异常，可以点击“清除 CDP 缓存”删除项目根目录下的 `.edge_cdp_profile/` 和 `.chrome_cdp_profile/`，然后重新打开调试浏览器登录。
...
5. 点击“开始抓取并导出”。
6. 预检查通过后继续任务。
7. 在候选卡片中人工选择需要入选的帖子；如果原帖有图片，会显示最多 3 张缩略图，并标注剩余图片数量。
8. 等待图片下载和文件导出完成。
...
- 输出清理只处理 `output/` 下的报告目录；取消或失败的未完成任务会同步清理项目根目录的 `cache/<运行ID>/`。
...
- 调试浏览器的独立登录缓存保存在 `.edge_cdp_profile/` 和 `.chrome_cdp_profile/`，可通过页面“清除 CDP 缓存”按钮删除。
