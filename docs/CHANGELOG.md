...
- 人工筛选候选卡片支持预选帖缩略图：每帖最多缓存 3 张缩略图，并在原帖图片超过 3 张时标注剩余数量。
- 新增 `thumbnails` 任务阶段，位于评分之后、人工筛选之前；任务日志会输出缩略图准备、缓存命中、下载成功和下载失败明细。
- 新增 `cache/<run_id>/candidate_thumbnails/` 缓存目录，缩略图复用本次任务根缓存，不写入最终 output。
- 导出结果列表新增长图预览入口，可像 Markdown 预览一样在右侧面板打开长图页面。
- Cookie 推荐方式新增“清除 CDP 缓存”按钮，可删除工具根目录下的 `.edge_cdp_profile/` 和 `.chrome_cdp_profile/`。
- 新增 `/api/cookie/clear-cdp-cache`，清理前会尝试关闭调试浏览器，并限制只能删除固定 CDP profile 目录。
- 任务取消和任务失败后自动清理未完成的 output 运行目录和对应根缓存目录。
- 任务状态支持“预检查结果先展开并收起，随后任务状态展开并自动收起”的展示节奏。

### Changed

- 评论分析与评分优化为复用预处理后的评分配置，并减少评论树重复遍历；计算结果保持不变。
- 任务状态收起后仅展示当前 active/waiting/failed/cancelled 阶段，仍随后台进度实时更新。
- 任务状态展开/收起改为显式测量起止高度，动画与缩回过程保持一致。
- 任务日志悬浮窗展开/收起动画改为更短的位移缩放过渡，并支持减少动态效果偏好。
- 候选序列化优先使用缩略图 URL，缺失时再回退到已下载原图路径。

### Fixed

- 修复任务状态收起时因为 Grid 容器锁高导致没有收起动画的问题。
- 修复失败任务和取消任务可能残留未完成 output 目录及对应 cache 的问题。
- 修复任务启动后预检查结果和任务状态同时展开、视觉节奏混乱的问题。
- 修复评论分析中嵌套评论遍历重复造成的性能浪费。
