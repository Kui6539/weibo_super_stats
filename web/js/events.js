window.WeiboEvents = {
  bind({
    ...
    // Run one binding group, reporting rather than aborting on failure.
    // These calls used to be a single straight run: if index.html lost or
    // renamed one id, the TypeError took out every binding after it -- start,
    // cancel, logs, preflight -- with nothing visible on the page to say so.
    const bindGroup = (label, fn) => {
      try {
        fn();
      } catch (err) {
        console.error(`[WeiboEvents] 绑定「${label}」失败，该区域的控件将无响应：`, err);
      }
    };
